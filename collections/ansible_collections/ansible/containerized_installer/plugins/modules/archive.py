    ...
    return os.path.join(
        os.path.dirname(
            os.path.commonprefix(
                [os.path.join(os.path.dirname(p), empty) for p in paths]
            )
        ),
        empty,
    )
...
def is_archive(path):
    return re.search(
        rb"\.(tar|tar\.(gz|bz2|xz)|tgz|tbz2|zip)$",
...
class Archive(object):
    def __init__(self, module):
        ...
        self.destination = (
            _to_bytes(module.params["dest"]) if module.params["dest"] else None
        )
        ...
        if not self.paths:
            module.fail_json(
                ...
                expanded_exclude_paths=_to_native(
                    b", ".join(self.expanded_exclude_paths)
                ),
        ...
        if not self.must_archive:
            self.must_archive = any(
                [has_globs, os.path.isdir(self.paths[0]), len(self.paths) > 1]
            )
    ...
    def add_single_target(self, path):
        if self.format in ("zip", "tar"):
            ...
            try:
                f_out = self._open_compressed_file(
                    _to_native_ascii(self.destination), "wb"
                )
                ...
                self.module.fail_json(
                    ...
                    dest=_to_native(self.destination),
                    msg="Unable to write to compressed file: %s" % _to_native(e),
    ...
    def add_targets(self):
        ...
        if self.errors:
            self.module.fail_json(
                ...
                % (_to_native(self.destination), "; ".join(self.errors))
            )
    ...
    def remove_single_target(self, path):
        try:
            ...
            self.module.fail_json(
                path=_to_native(path),
                msg="Unable to remove source file: %s" % _to_native(e),
                exception=format_exc(),
            )
    ...
    def result(self):
        return {
            ...
            "expanded_exclude_paths": [
                _to_native(p) for p in self.expanded_exclude_paths
            ],
        }

    def _check_removal_safety(self):
        for path in self.paths:
            if os.path.isdir(path) and self.destination.startswith(
                os.path.join(path, b"")
            ):
                self.module.fail_json(
...
class ZipArchive(Archive):
    ...
    def open(self):
        self.file = zipfile.ZipFile(
            _to_native_ascii(self.destination), "w", zipfile.ZIP_DEFLATED, True
        )
    ...
    def open(self):
        if self.format in ("gz", "bz2"):
            self.file = tarfile.open(
                _to_native_ascii(self.destination), "w|" + self.format
            )
        ...
        try:
            if self.format == "xz":
                with lzma.open(_to_native_ascii(path), "r") as f:
                    ...
                    checksums = set(
                        (info.name, info.chksum) for info in archive.getmembers()
                    )
                    archive.close()
            else:
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            format=dict(
                type="str", default="gz", choices=["bz2", "gz", "tar", "xz", "zip"]
            ),
            dest=dict(type="path"),
    ...
    if not archive.has_targets():
        if archive.destination_exists():
            archive.destination_state = (
                STATE_ARCHIVED if is_archive(archive.destination) else STATE_COMPRESSED
            )
        ...
        if check_mode:
            ...
            archive.add_targets()
            archive.destination_state = (
                STATE_INCOMPLETE if archive.has_unfound_targets() else STATE_ARCHIVED
            )
